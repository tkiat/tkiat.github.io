...
describe('check number of tablist, tab, and panels', () => {
  test.each([0, 3, 5])('test ', (n) => {
    render(<Tabs initIndex={0} cleanup={cleanup} updateRef={update} items={getDupeItems(n)} />)
...
test('cleanup function must be called once after unmount', () => {
  ...
  const { unmount } = render(<Tabs initIndex={0} cleanup={mockCleanup} updateRef={update} items={getDupeItems(2)} />)
...
test('updateRef function must be called with tab index of pressed button', () => {
  const numItems = 5
  const mockUpdate = jest.fn()
  render(<Tabs initIndex={0} cleanup={cleanup} updateRef={mockUpdate} items={getDupeItems(numItems)} />)
